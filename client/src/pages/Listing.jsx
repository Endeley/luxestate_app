...
function Listing() {
    ...
    useEffect(() => {
        const fetchListing = async () => {
            try {
                setLoading(true);
                // Check if listingId is available
                if (params.listingId) {
                    ...
                    // Handle the case when listingId is not available
                    setError(true);
                    setLoading(false);
                }
            } catch (error) {
                ...
                <div>
                    <Swiper navigation>
                        {listing.imageUrls.map((url) => (
                            <SwiperSlide key={url}>
                                <div className='h-[550px] bg-no-repeat bg-center ' style={{ background: `url(${url}) center no-repeat`, backgroundSize: 'cover', backgroundPosition: 'fit' }}></div>
                            </SwiperSlide>
                        ))}
                    </Swiper>
                    ...
                    <div className='flex flex-col max-w-4xl mx-auto p-3 my-7 gap-4'>
                        ...
                        <p className='text-slate-800 '>
                            <span className='font-semibold tracking-wide text-black'>Description - </span>
                            {listing.description}{' '}
                        </p>
                        <u className='text-green-900 font-semibold text-sm flex flex-wrap items-center gap-4 sm:gap-6'>
                            {' '}
